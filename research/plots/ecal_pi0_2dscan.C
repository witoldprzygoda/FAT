...
// --- Build N_all/N_++/N_-- -> signal for one (E,θ) cell ----------------
...
void ecal_pi0_2dscan(const char* flavour = "rec") {
    ...
    const std::string out_root = "ecal_pi0_2dscan_" + fl + ".root";
    const std::string out_pdf  = "plots/output/ecal_pi0_2dscan_" + fl + ".pdf";
    ...
    for (int ei = 0; ei < nE; ++ei) {
        for (int ti = 0; ti < nTh; ++ti) {
            ...
            TH1D* h_signal = buildSignal(t_all, t_pp, t_mm, mass_var,
                                         el, eh, tl, th, tag);
    ...
    drawHeat(h_mu,    "ecal_pi0_2dscan_mu",    "COLZ TEXT45");
    drawHeat(h_s,     "ecal_pi0_2dscan_scale", "COLZ TEXT45");
    drawHeat(h_sig,   "ecal_pi0_2dscan_sigma", "COLZ TEXT45");
    drawHeat(h_yield, "ecal_pi0_2dscan_yield", "COLZ TEXT");
    ...
    std::cout << "\nWrote:\n"
              ...
              << "  plots/output/ecal_pi0_2dscan_{mu,scale,sigma,yield}_" << fl << ".{pdf,png}\n";
}
