...
void fit_eta(const char* flavour = "rec",
    ...
    const std::string pdf_multi     = "plots/output/fit_eta_" + fl + sfx + "_all.pdf";
    const std::string pdf_multi_res = "plots/output/fit_eta_" + fl + sfx + "_residual_all.pdf";

    const std::string fres_path = "fit_results_eta_" + fl + sfx + ".root";
    ...
    double alpha_ref = -1.0;
    {
        ...
        if (h_sig) {
            ...
            FitRes r = fitOneHist(h_sig, hname, ctitle.Data(),
                                  fl + sfx + "_full", canvases, canvases_res);
    ...
    for (int i = 0; i < n_slices; ++i) {
        ...
        FitRes r = fitOneHist(h_sig, hname, ctitle.Data(),
                              fl + sfx + "_slice_" + suff, canvases, canvases_res,
                              alpha_ref);
