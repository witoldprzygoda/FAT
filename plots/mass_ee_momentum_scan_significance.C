// mass_ee_momentum_scan_significance.C — 2D scan of (ep_p_rec, em_p_rec)
// MOMENTUM cuts to maximize CB-subtracted signal significance, with the
// previously optimized RICH cuts (padnum and richmatchqualitynorm) applied
// as a PRE-FILTER on every event.
//
// Pre-filter (applied to every event in all 3 channels BEFORE the momentum
// scan; values are the TOP cuts found in the dedicated sister scans):
//   ep_rich_padnum            >= kEpPadnumMin   (= 7)
//   em_rich_padnum            >= kEmPadnumMin   (= 8)
//   ep_richmatchqualitynorm   <= kEpQualMax     (= 2)
//   em_richmatchqualitynorm   <= kEmQualMax     (= 10)
// Like-sign sync: EpEp uses ep-leg cuts on BOTH legs (both are e+);
// EmEm uses em-leg cuts on both legs.
//
// What is scanned (ASYMMETRIC ep / em):
//   • EpEm pair (signal candidates):
//       ep_p_rec >  ep_p_cut    AND   em_p_rec >  em_p_cut
//   • EpEp pair (like-sign + +, CB): SYNCHRONIZED on ep_p_cut
//       (both legs must satisfy p > ep_p_cut — both are e+)
//   • EmEm pair (like-sign − −, CB): SYNCHRONIZED on em_p_cut
//       (both legs must satisfy p > em_p_cut — both are e−)
//
// Default cut grid: cut ∈ {0, 10, 20, …, 300} MeV/c (31 values per leg →
// 961 combinations). Strict inequality: cut = 0 ⇒ p > 0 ⇒ all real
// momenta pass; cut = 300 ⇒ only p > 300 MeV/c passes.
//
...
//   1.   plots/output/scan_momentum_heatmap_significance.{pdf,png}
//        — 2×4 grid (rows: PT3/PT2; cols: A/B/C/D) of per-trigger
//          significance vs (ep_p_cut, em_p_cut). Best grid-point
//          (shared between PT2 and PT3 rows) marked with a ★.
//
//   2-5. plots/output/scan_momentum_best_<window>.{pdf,png}
...
//
// Usage:
//   root -l -b -q plots/mass_ee_momentum_scan_significance.C
//   root -l -b -q 'plots/mass_ee_momentum_scan_significance.C(0, 300, 10)'
//
...
// -------- Pre-filter: TOP cuts from the padnum + matchqualnorm scans --
constexpr float kEpPadnumMin = 7.0f;     // ep_rich_padnum            >=
constexpr float kEmPadnumMin = 8.0f;     // em_rich_padnum            >=
constexpr float kEpQualMax   = 2.0f;     // ep_richmatchqualitynorm   <=
constexpr float kEmQualMax   = 10.0f;    // em_richmatchqualitynorm   <=
...
void mass_ee_momentum_scan_significance(int cut_min  = 0,
                                        int cut_max  = 300,
                                        int cut_step = 10) {
    gStyle->SetOptStat(0);
    gStyle->SetOptFit(0);
    ...
    if (cut_step <= 0) {
        std::cerr << "cut_step must be positive\n"; return;
    }
    const int n_cuts = (cut_max - cut_min) / cut_step + 1;
    // Helper: cut value for grid index i.
    auto cutVal = [cut_min, cut_step](int i) { return cut_min + i * cut_step; };
    std::cout << "Scanning ep_p_rec, em_p_rec on [" << cut_min << ", " << cut_max
              << "] MeV/c step " << cut_step << "  (n_cuts=" << n_cuts
              << ", " << n_cuts * n_cuts << " combinations)\n"
              << "Convention: p_rec > cut_value passes.\n"
              << "Pre-filter (applied to every event):\n"
              << "  ep_rich_padnum         >= " << kEpPadnumMin << "\n"
              << "  em_rich_padnum         >= " << kEmPadnumMin << "\n"
              << "  ep_richmatchqualitynorm <= " << kEpQualMax  << "\n"
              << "  em_richmatchqualitynorm <= " << kEmQualMax  << "\n";
    ...
    for (int i = 0; i < n_cuts; ++i) {
        for (int t = 0; t < NUM_TRG; ++t) {
            h_epep[i][t] = new TH1D(Form("h_epep_p%d_t%d", cutVal(i), t),
                                    "", kNb, kXmin, kXmax);
            h_epep[i][t]->Sumw2();
            h_epep[i][t]->SetDirectory(nullptr);
            h_emem[i][t] = new TH1D(Form("h_emem_p%d_t%d", cutVal(i), t),
                                    "", kNb, kXmin, kXmax);
            h_emem[i][t]->Sumw2();
        ...
        for (int j = 0; j < n_cuts; ++j) {
            for (int t = 0; t < NUM_TRG; ++t) {
                h_epem[i][j][t] = new TH1D(
                    Form("h_epem_p%d_p%d_t%d", cutVal(i), cutVal(j), t),
                    "", kNb, kXmin, kXmax);
                h_epem[i][j][t]->Sumw2();
    ...
    Long64_t n_em_total = 0, n_em_pre = 0, n_em_use = 0;
    {
        ...
        TTreeReaderValue<float> ep_pad(r, "ep_rich_padnum");
        TTreeReaderValue<float> em_pad(r, "em_rich_padnum");
        TTreeReaderValue<float> ep_q (r, "ep_richmatchqualitynorm");
        TTreeReaderValue<float> em_q (r, "em_richmatchqualitynorm");
        TTreeReaderValue<float> ep_p (r, "ep_p_rec");
        TTreeReaderValue<float> em_p (r, "em_p_rec");

        while (r.Next()) {
            ++n_em_total;
            const int trg = trgFromBit(static_cast<int>(*trigb));
            if (trg < 0) continue;
            // Pre-filter (RICH cuts from sister scans).
            if (*ep_pad < kEpPadnumMin) continue;
            if (*em_pad < kEmPadnumMin) continue;
            if (*ep_q   > kEpQualMax  ) continue;
            if (*em_q   > kEmQualMax  ) continue;
            ++n_em_pre;
            // Momentum scan: cut value < p_rec passes.
            // Largest passing idx = ceil((p - cut_min) / step) - 1.
            const double P_ep = *ep_p;
            const double P_em = *em_p;
            int max_e = static_cast<int>(
                std::ceil((P_ep - cut_min) / static_cast<double>(cut_step))) - 1;
            int max_m = static_cast<int>(
                std::ceil((P_em - cut_min) / static_cast<double>(cut_step))) - 1;
    ...
    std::cout << "  scanned " << n_em_total
              << ", pre-filter passed " << n_em_pre
              << ", used in scan " << n_em_use
    ...
    Long64_t n_pp_total = 0, n_pp_pre = 0, n_pp_use = 0;
    {
        TFile f("output_epep_exp.root", "READ");
        TTreeReader r("dilepton_nt", &f);
        TTreeReaderValue<float> mee  (r, "m_ee");
        TTreeReaderValue<float> trigb(r, "trigbit");
        TTreeReaderValue<float> l1_pad(r, "ep_rich_padnum");
        TTreeReaderValue<float> l2_pad(r, "em_rich_padnum");
        TTreeReaderValue<float> l1_q (r, "ep_richmatchqualitynorm");
        TTreeReaderValue<float> l2_q (r, "em_richmatchqualitynorm");
        TTreeReaderValue<float> l1_p (r, "ep_p_rec");
        TTreeReaderValue<float> l2_p (r, "em_p_rec");

        while (r.Next()) {
            ++n_pp_total;
            const int trg = trgFromBit(static_cast<int>(*trigb));
            if (trg < 0) continue;
            // Pre-filter: both legs are e+, both must satisfy ep-cut.
            if (*l1_pad < kEpPadnumMin || *l2_pad < kEpPadnumMin) continue;
            if (*l1_q   > kEpQualMax   || *l2_q   > kEpQualMax  ) continue;
            ++n_pp_pre;
            // Momentum scan: cut < min(P1, P2).
            const double P = std::min<double>(*l1_p, *l2_p);
            int max_c = static_cast<int>(
                std::ceil((P - cut_min) / static_cast<double>(cut_step))) - 1;
            max_c = std::min(max_c, n_cuts - 1);
            if (max_c < 0) continue;
            ++n_pp_use;
            const double m = *mee;
            for (int e = 0; e <= max_c; ++e) {
                h_epep[e][trg]->Fill(m);
            }
        }
    }
    std::cout << "  scanned " << n_pp_total
              << ", pre-filter passed " << n_pp_pre
              << ", used in scan " << n_pp_use << "\n";

    // ============================================================================
    // Pass 3 — EmEm: both legs share em_cut (synchronized).
    // ============================================================================
    std::cout << "Pass 3/3 — EmEm (output_emem_exp.root)...\n";
    Long64_t n_mm_total = 0, n_mm_pre = 0, n_mm_use = 0;
    {
        ...
        while (r.Next()) {
            ...
            const int trg = trgFromBit(static_cast<int>(*trigb));
            if (trg < 0) continue;
            // Pre-filter: both legs are e−, both must satisfy em-cut.
            if (*l1_pad < kEmPadnumMin || *l2_pad < kEmPadnumMin) continue;
            if (*l1_q   > kEmQualMax   || *l2_q   > kEmQualMax  ) continue;
            ++n_mm_pre;
            const double P = std::min<double>(*l1_p, *l2_p);
            int max_c = static_cast<int>(
                std::ceil((P - cut_min) / static_cast<double>(cut_step))) - 1;
    ...
    std::cout << "  scanned " << n_mm_total
              << ", pre-filter passed " << n_mm_pre
              << ", used in scan " << n_mm_use << "\n";
    ...
    for (size_t w = 0; w < kWindows.size(); ++w) {
        for (int t = 0; t < NUM_TRG; ++t) {
            auto* h = new TH2D(
                ...
                Form("Significance — %s / %s;ep_p_rec > [MeV/c];em_p_rec > [MeV/c];S / #sqrt{S + 2B}",
        ...
        for (int mi = 0; mi < n_cuts; ++mi) {
            ...
            for (size_t w = 0; w < kWindows.size(); ++w) {
                ...
                if (sum_sig > best[w].signif_sum) {
                    best[w].ep_cut     = cutVal(ei);
                    best[w].em_cut     = cutVal(mi);
                    best[w].S          = Sw;
                    best[w].B          = Bw;
                    best[w].signif     = sigw;
    ...
    TCanvas* c_heat = new TCanvas("c_scan_momentum_heatmap",
        "Significance landscape (p_rec scan) — 4 windows × 2 triggers",
        1900, 900);
    ...
    c_heat->SaveAs("plots/output/scan_momentum_heatmap_significance.pdf");
    c_heat->SaveAs("plots/output/scan_momentum_heatmap_significance.png");
    std::cout << "  saved: plots/output/scan_momentum_heatmap_significance.{pdf,png}\n";
    ...
    for (size_t w = 0; w < kWindows.size(); ++w) {
        ...
        TCanvas* c = new TCanvas(
            Form("c_momentum_best_%s", kWindows[w].short_id.c_str()),
            Form("Best %s (p_rec scan) — ep_p_cut=%d em_p_cut=%d MeV/c",
        ...
        auto drawMeePanel = [&](int pad_idx, int t) {
            ...
            h_all[t]->SetTitle(Form(
                "m_{ee} at %s (best cuts ep_p>%d em_p>%d MeV/c);"
        ...
        const std::string base = Form(
            "plots/output/scan_momentum_best_%s",
