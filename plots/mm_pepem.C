...
// pe+e- compound uses FWD proton candidate
...
void mm_pepem() {
    ...
    TH1D *all1, *cb1, *sig1;
    std::tie(all1, cb1, sig1) = pu.drawSignal("pepem_nt", "mm_pepem_mass",
                                                100, 0, 2.0, "oa>9 && m_ee>0.14",
    ...
    TH1D *all2, *cb2, *sig2;
    std::tie(all2, cb2, sig2) = pu.drawSignal("pepem_nt", "m_ee",
                                                160, 0, 0.8,
    ...
    // --- 3. Invariant mass of pe+e- (OA > 9, M_ee > 0.14, MM proton window) ---
    TH1D *all3, *cb3, *sig3;
    std::tie(all3, cb3, sig3) = pu.drawSignal("pepem_nt", "pepem_mass",
                                                50, 0.8, 1.8,
    ...
    printIntegrals("M(pe+e-) (OA>9, M_ee>0.14, MM proton) full range", all3, cb3, sig3);
