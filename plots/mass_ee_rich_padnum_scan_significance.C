...
void mass_ee_rich_padnum_scan_significance(int cut_min = 5,
                                           int cut_max = 20) {
    gStyle->SetOptStat(0);
    gStyle->SetOptFit(0);
    ...
    TCanvas* c_heat = new TCanvas("c_scan_heatmap_significance",
        "Significance landscape — 4 windows × 2 triggers",
        1900, 900);
    ...
    c_heat->SaveAs("plots/output/scan_rich_padnum_heatmap_significance.pdf");
    c_heat->SaveAs("plots/output/scan_rich_padnum_heatmap_significance.png");
    std::cout << "  saved: plots/output/scan_rich_padnum_heatmap_significance.{pdf,png}\n";
    ...
    for (size_t w = 0; w < kWindows.size(); ++w) {
        ...
        TCanvas* c = new TCanvas(
            Form("c_best_%s", kWindows[w].short_id.c_str()),
            Form("Best %s — ep_cut=%d, em_cut=%d",
        ...
        const std::string base = Form(
            "plots/output/scan_rich_padnum_best_%s",
