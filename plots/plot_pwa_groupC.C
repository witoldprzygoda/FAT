/**
 * @file plot_pwa_groupC.C
 * @brief Plot PWA Group C: Invariant mass distributions
 *
 * Displays pwa_ppip_m, pwa_npip_m, pwa_pn_m from three files
 * File 1 (black): output_fwd_ppip.root
 * File 2 (blue): output_fwd_ppip_full.root
 * File 3 (red): output_fwd_ppip_neutroncut.root
 */

void plot_pwa_groupC() {
    gStyle->SetOptStat(0);
    ...
    // Histogram names for Group C
    const char* hist_names[3] = {"pwa/pwa_ppip_m", "pwa/pwa_npip_m", "pwa/pwa_pn_m"};
    const char* titles[3] = {"M(p#pi^{+})", "M(n#pi^{+})", "M(pn)"};

    // Create canvas with 3 horizontal pads
    TCanvas* c1 = new TCanvas("c1", "PWA Group C: Invariant mass distributions", 1800, 600);
    c1->Divide(3, 1);
    ...
    c1->Update();
    // c1->SaveAs("plot_pwa_groupC.pdf");
    // c1->SaveAs("plot_pwa_groupC.png");
