...
//
// All output filenames are suffixed with "_cor" to keep figures from the two
...
void printIntegral_cor(const char* label, TH1D* h) {
    std::cout << "\n=== " << label << " ===\n";
...
void pippimepem_spectra_cor() {
    ...
    auto* h1 = pu.drawNtupleSingle(
        ...
        160, 0.0, 0.8, wcut(""),
        ";M_{e^{+}e^{-}} [GeV/c^{2}];a.u.");
    auto* cv1 = pu.drawSingle(h1, "M_{e^{+}e^{-}} (cor)", "c_m_ee_cor", /*logy=*/true);
    pu.save(cv1, "m_ee_cor");
    printIntegral_cor("M(e+e-) cor", h1);
    ...
    auto* cv2 = pu.drawSingle(h2, "M_{#pi^{+}#pi^{-}} (cor)", "c_m_pippim_cor");
    pu.save(cv2, "m_pippim_cor");
    printIntegral_cor("M(pi+pi-) cor", h2);
    ...
    printIntegral_cor("M(pi+pi-e+e-) cor", h3);
    ...
    auto* cv4 = pu.drawSingle(h4, "MM(#pi^{+}#pi^{-}) (cor)", "c_mm_pippim_cor");
    pu.save(cv4, "mm_pippim_cor");
    printIntegral_cor("MM(pi+pi-) cor", h4);
    ...
    printIntegral_cor("MM(pi+pi-e+e-) cor", h5);
    ...
    // --- 5a-5c. MM observables after cut_2d -------------------------------
    {
        ...
        printIntegral_cor("MM(e+e-) after cut_2d cor", h);
    }
    {
        auto* h = pu.drawNtupleSingle(
            NT, "mm_pippim",
        ...
        auto* cv = pu.drawSingle(h, "MM(#pi^{+}#pi^{-}) after cut_2d (cor)",
                                 "c_mm_pippim_cut2d_cor");
        pu.save(cv, "mm_pippim_cut2d_cor");
        printIntegral_cor("MM(pi+pi-) after cut_2d cor", h);
    }
    {
        auto* h = pu.drawNtupleSingle(
            NT, "mm_pippimepem",
    ...
    printIntegral_cor("M(pi+pi-e+e-) after selection cor", h6);
    ...
    // --- 6a. Same as plot 6 but with cut_2d additionally applied ----------
    {
        ...
        printIntegral_cor("M(pi+pi-e+e-) after selection + cut_2d cor", h);
    }
    ...
    auto drawSlice = [&](double lo, double hi, const std::string& tag, bool cut2d = false) {
        ...
        lbl << "M(pi+pi-e+e-) sel" << (cut2d ? "+cut_2d" : "")
            << ", MM in [" << lo << ", " << hi << "] cor";
        printIntegral_cor(lbl.str().c_str(), h);
    };
