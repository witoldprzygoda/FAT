...
// (simulation, η Dalitz region).
//
//   1) yield_eta_<fl>_vs_oa.{pdf,png}
//      η signal counts vs opening angle, three series (μ ± 1σ, ±2σ, ±3σ).
//      X — slice center (deg);    X-bars = slice half-width = 0.25°.
//      Y — yield(data − bg);      Y-bars = IntegralAndError statistical error
//                                          (Sumw2-aware, sim_genweight).
//
//   2) mu_eta_<fl>_vs_oa.{pdf,png}
//      Gaussian peak position μ vs opening angle.
//      X — slice center (deg);    X-bars = slice half-width.
//      Y — μ from Gauss fit;      Y-bars = ±σ_G (peak width).
//
// Usage (from research/):
//   root -l -b -q plots/yields_eta.C            # REC (default)
//   root -l -b -q 'plots/yields_eta.C("cor")'   # COR
//   root -l -b -q 'plots/yields_eta.C("tru")'   # TRU
...
void yields_eta(const char* flavour = "rec",
    ...
    const std::string fpath = "fit_results_eta_" + fl + sfx + "_sim.root";
    ...
    c1->Update();
    const std::string out1 = "plots/output/yield_eta_" + fl + sfx + "_sim_vs_oa";
    c1->SaveAs((out1 + ".pdf").c_str());
    c1->SaveAs((out1 + ".png").c_str());

    // --- Plot 2: μ vs OA (vertical bars = ±σ_G) -----------------------------
    ...
    gm->SetTitle(TString::Format(
        "#eta peak position vs opening angle (%s, sim);"
        "OA(e^{+}e^{-}) [deg];"
        "#mu_{G} [GeV/c^{2}]  (vertical bars = #pm#sigma_{G})",
        fl.c_str()));
    ...
    leg2->AddEntry(gm,   "fit #mu (bars = #pm#sigma_{G})", "lpe");
    leg2->AddEntry(lref, TString::Format("PDG #eta = %.4f", kEtaPDG).Data(), "l");
    leg2->Draw();

    c2->Update();
    const std::string out2 = "plots/output/mu_eta_" + fl + sfx + "_sim_vs_oa";
