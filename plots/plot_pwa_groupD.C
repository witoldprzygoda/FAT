/**
 * @file plot_pwa_groupD.C
 * @brief Plot PWA Group D: Helicity angle distributions
 *
 * Displays pwa_pip_helicity, pwa_pipn_helicity, pwa_n_helicity from three files
 * File 1 (black): output_fwd_ppip.root
 * File 2 (blue): output_fwd_ppip_full.root
 * File 3 (red): output_fwd_ppip_neutroncut.root
 */

void plot_pwa_groupD() {
    gStyle->SetOptStat(0);
    ...
    // Histogram names for Group D
    const char* hist_names[3] = {"pwa/pwa_pip_helicity", "pwa/pwa_pipn_helicity", "pwa/pwa_n_helicity"};
    const char* titles[3] = {"#pi^{+} helicity angle", "#pi^{+}n helicity angle", "Neutron helicity angle"};

    // Create canvas with 3 horizontal pads
    TCanvas* c1 = new TCanvas("c1", "PWA Group D: Helicity angle distributions", 1800, 600);
    c1->Divide(3, 1);
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        h1->GetXaxis()->SetTitle("cos#theta_{helicity}");
    ...
    c1->Update();
    // c1->SaveAs("plot_pwa_groupD.pdf");
    // c1->SaveAs("plot_pwa_groupD.png");
