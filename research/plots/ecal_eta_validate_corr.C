                    ...
                    const std::string& tag)
{
    ...
    // Use the closed-form correction (no gamma_D required), so this macro
    // works on the existing sim ntuple even before gamma_D is added.
    float m_ee=0, m_eg=0, ecal_q=0, ne_E=0, ne_th=0, oa=0, w=1.0f;
    ...
    for (Long64_t ev = 0; ev < N; ++ev) {
        ...
        const double m_corr = look.m_corrected_closed(m_ee, m_eg, E_GeV, ne_th);
        h_uncorr->Fill(m_eg,   w);
        h_corr  ->Fill(m_corr, w);
    }
    f->Close();
}

// SIM has no like-sign CB — the filled histogram IS the signal.
// Helper kept as a clone wrapper to mirror the experiment-side macro.
TH1D* makeSignal(TH1D* h_all, const std::string& tag) {
...
void ecal_eta_validate_corr(const char* flavour = "rec") {
    ...
    std::cout << "ECAL η validation (SIM), flavour=" << fl << "\n";
    fillEtaChannel("../output_epem_sim.root", look, h_all_u, h_all_c, "sim");

    // SIM: filled hist = signal directly.
    TH1D* h_sig_u = makeSignal(h_all_u, "u_" + fl);
    TH1D* h_sig_c = makeSignal(h_all_c, "c_" + fl);
    ...
    c->Update();
    const std::string base = "plots/output/ecal_eta_validate_corr_" + fl + "_sim";
    c->SaveAs((base + ".pdf").c_str());
    c->SaveAs((base + ".png").c_str());

    // Save per-stream hists for later inspection.
    TFile* fout = TFile::Open(("ecal_eta_validate_corr_" + fl + "_sim.root").c_str(),
                              "RECREATE");
    h_sig_u->Write();
    h_sig_c->Write();
    h_all_u->Write();
