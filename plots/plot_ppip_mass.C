/**
 * @file plot_ppip_mass.C
 * @brief Plot ppip invariant mass comparison
 *
 * Displays ppip_inv_mass from three files
...
void plot_ppip_mass() {
    ...
    TH1D* h1 = (TH1D*)f1->Get("ppip/ppip_inv_mass");
    TH1D* h2 = (TH1D*)f2->Get("ppip/ppip_inv_mass");
    TH1D* h3 = (TH1D*)f3->Get("ppip/ppip_inv_mass");

    if (!h1 || !h2 || !h3) {
        std::cerr << "Error: Could not get histogram ppip_inv_mass!" << std::endl;
        return;
    }

    // Create canvas
    TCanvas* c1 = new TCanvas("c1", "M(p#pi^{+}) Invariant Mass Comparison", 800, 600);
    ...
    h1->SetTitle("M(p#pi^{+}) Invariant Mass");
    h1->GetXaxis()->SetTitle("M(p#pi^{+}) [GeV/c^{2}]");
    ...
    c1->Update();
    // c1->SaveAs("plot_ppip_mass.pdf");
    // c1->SaveAs("plot_ppip_mass.png");
