// Compare μ vs OA after combined π⁰+η calibration: OA>4° cal-cut vs OA>0° cal-cut.
// Reads four sim fit_results files (two per channel) and overlays them.
...
static void plot2(const std::string& title, double pdg,
                  const OAGraph& a, const OAGraph& b,
                  ...
                  const std::string& out)
{
    ...
    auto* ga = build(a, kBlue+1,  20);
    auto* gb = build(b, kRed+1,   21);

    auto* cv = new TCanvas(("c_"+out).c_str(), out.c_str(), 1100, 700);
    cv->SetMargin(0.13, 0.05, 0.12, 0.08); cv->SetGrid();
    ga->SetTitle(TString::Format(
        "%s;OA(e^{+}e^{-}) [deg];#mu_{CB} [GeV/c^{2}]", title.c_str()));
    ga->Draw("AP");
    ga->GetXaxis()->SetLimits(0.0, 15.0);
    ga->GetYaxis()->SetRangeUser(yLo, yHi);
    gb->Draw("P SAME");
...
void mu_combined_oa4_vs_oa0() {
    gStyle->SetOptStat(0);
    gSystem->mkdir("plots/output", kTRUE);

    // SIM
    plot2("#pi^{0} #mu vs OA (SIM, combined #pi^{0}+#eta)", kPi0PDG,
          readMuVsOA("fit_results_rec_ecalcor_combined_oa4_sim.root"),
          readMuVsOA("fit_results_rec_ecalcor_combined_oa0_sim.root"),
          0.115, 0.165,
          "calib OA>4#circ", "calib OA>0#circ",
          "mu_pi0_combined_sim_oa4_vs_oa0");

    plot2("#eta #mu vs OA (SIM, combined #pi^{0}+#eta)", kEtaPDG,
          readMuVsOA("fit_results_eta_rec_ecalcor_combined_oa4_sim.root"),
          readMuVsOA("fit_results_eta_rec_ecalcor_combined_oa0_sim.root"),
          0.49, 0.62,
          "calib OA>4#circ", "calib OA>0#circ",
          "mu_eta_combined_sim_oa4_vs_oa0");

    // EXP
    plot2("#pi^{0} #mu vs OA (EXP, combined #pi^{0}+#eta)", kPi0PDG,
          readMuVsOA("fit_results_rec_ecalcor_combined_oa4.root"),
          readMuVsOA("fit_results_rec_ecalcor_combined_oa0.root"),
          0.115, 0.165,
          "calib OA>4#circ", "calib OA>0#circ",
          "mu_pi0_combined_exp_oa4_vs_oa0");

    plot2("#eta #mu vs OA (EXP, combined #pi^{0}+#eta)", kEtaPDG,
          readMuVsOA("fit_results_eta_rec_ecalcor_combined_oa4.root"),
          readMuVsOA("fit_results_eta_rec_ecalcor_combined_oa0.root"),
          0.49, 0.62,
          "calib OA>4#circ", "calib OA>0#circ",
          "mu_eta_combined_exp_oa4_vs_oa0");
}
