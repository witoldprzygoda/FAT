/**
 * @file ImprovedManager.h
 * @brief Improved histogram and ntuple manager using modern C++ patterns
 *
 * This is a modernized version of Manager that:
...
#ifndef IMPROVED_MANAGER_H
#define IMPROVED_MANAGER_H
...
// ============================================================================
// ImprovedManager: Modern histogram/ntuple manager
// ============================================================================
...
public:
    ...
    ImprovedManager(const ImprovedManager&) = delete;
    ImprovedManager& operator=(const ImprovedManager&) = delete;

    // Allow move semantics
    ImprovedManager(ImprovedManager&&) = default;
    ImprovedManager& operator=(ImprovedManager&&) = default;
    ...
    void openFile(const std::string& filename, const std::string& option = "RECREATE") {
        if (file_ && file_->IsOpen()) {
            throw std::runtime_error("ImprovedManager::openFile() - File already open!");
        }

        file_ = std::make_unique<TFile>(filename.c_str(), option.c_str());

        if (!file_->IsOpen()) {
            throw std::runtime_error("ImprovedManager::openFile() - Failed to open file: " + filename);
        }

        std::cout << "ImprovedManager: Opened file '" << filename << "' with option '" << option << "'\n";
    }
    ...
    void closeFile() {
        if (!file_ || !file_->IsOpen()) {
            throw std::runtime_error("ImprovedManager::closeFile() - No file is open!");
        }
        ...
        file_->Close();
        std::cout << "ImprovedManager: File closed and all histograms written.\n";
    }
    ...
    {
        if (!file_ || !file_->IsOpen()) {
            throw std::runtime_error("ImprovedManager::createNtuple() - No file open! Call openFile() first.");
        }
