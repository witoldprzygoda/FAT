// mass_ee_momentum_flatness_scan_sim.C — 2D scan of (ep_p_rec, em_p_rec)
// LOWER-momentum cuts on the SIM sample to find combinations that FLATTEN
// the PT2/PT3 trigger-ratio modulation in m_ee_sim space.
//
// CRITICAL: a PRE-FILTER from the EXP TOP cuts (found in the exp padnum &
// quality scans) is applied to EVERY event BEFORE the momentum scan. This
// tests whether the EXP-derived RICH cuts also flatten the SIM PT2/PT3 ratio
// and whether an ADDITIONAL momentum cut on top can further improve the
// flatness.
//
// Pre-filter (applied to ep AND em leg of each pair):
//     ep_rich_padnum            >= 7
//     em_rich_padnum            >= 8
//     ep_richmatchqualitynorm   <= 2
//     em_richmatchqualitynorm   <= 10
//
// Differences vs the exp padnum scan:
...
//   ep_p_rec > ep_cut    AND    em_p_rec > em_cut       (STRICT inequality)
//   Default grid: cut ∈ [0, 300] step 10 MeV/c (31 values per leg
//   → 961 combinations).
//
// Mass windows:
...
//   plots/output/scan_momentum_flatness_sim_heatmap.{pdf,png}
//   plots/output/scan_momentum_flatness_sim_best_<window>.{pdf,png}  (×4)
//
...
//
// Usage:
//   root -l -b -q plots/mass_ee_momentum_flatness_scan_sim.C
//   root -l -b -q 'plots/mass_ee_momentum_flatness_scan_sim.C(0, 300, 10)'
//
...
// -------- Pre-filter constants (EXP TOP cuts from exp scans) --------
constexpr float kEpPadnumMin = 7.0f;
constexpr float kEmPadnumMin = 8.0f;
constexpr float kEpQualMax   = 2.0f;
constexpr float kEmQualMax   = 10.0f;
...
void mass_ee_momentum_flatness_scan_sim(int cut_min = 0, int cut_max = 300,
                                        int cut_step = 10) {
    gStyle->SetOptStat(0);
    gStyle->SetOptFit(0);
    ...
    std::cout << "Scanning ep_p_rec, em_p_rec on ["
              ...
              << "Convention: p_rec > cut_value passes (STRICT inequality).\n"
              << "Pre-filter (EXP TOP cuts, applied to every event):\n"
    ...
    // ============================================================================
    // Allocate histograms: h_epem[ei][mi][trg] over the momentum-cut grid.
    ...
    Long64_t n_total = 0, n_prefilter = 0, n_use = 0;
    {
        ...
        TTreeReaderValue<float> v_ep_pad(r, "ep_rich_padnum");
        TTreeReaderValue<float> v_em_pad(r, "em_rich_padnum");
        TTreeReaderValue<float> v_ep_q  (r, "ep_richmatchqualitynorm");
        TTreeReaderValue<float> v_em_q  (r, "em_richmatchqualitynorm");
        ...
        while (r.Next()) {
            ...
            // Pre-filter: EXP TOP cuts applied to every event.
            if (*v_ep_pad < kEpPadnumMin) continue;
            if (*v_em_pad < kEmPadnumMin) continue;
            if (*v_ep_q   > kEpQualMax  ) continue;
            if (*v_em_q   > kEmQualMax  ) continue;
            ++n_prefilter;

            // Momentum cuts: p_rec > cutVal(i) passes ⇒ event contributes
            // to all i in [0, max_idx] where cutVal(max_idx) < p_rec.
            // max_idx = ceil((p - cut_min)/step) - 1, capped at n_cuts-1.
    ...
    std::cout << "  scanned        " << n_total << "\n"
              << "  pre-filter ok  " << n_prefilter
              << "  (" << (100.0 * n_prefilter / std::max<Long64_t>(1, n_total))
              << " % survival)\n"
              << "  used in scan   " << n_use
              << "  (" << (100.0 * n_use / std::max<Long64_t>(1, n_total))
              << " % of scanned, "
              << (100.0 * n_use / std::max<Long64_t>(1, n_prefilter))
              << " % of pre-filter)\n";
    ...
    for (size_t w = 0; w < kWindows.size(); ++w) {
        h_chi2[w][0] = new TH2D(
            ...
            Form("Flatness #chi^{2}/ndf — %s;ep_p_rec > [MeV/c];em_p_rec > [MeV/c];#chi^{2}/ndf",
    ...
    for (size_t w = 0; w < kWindows.size(); ++w) {
        ...
        std::cout
            ...
            << pad(std::to_string(b.ep_cut), 3) << "  "
            << pad(std::to_string(b.em_cut), 3) << "  |  "
    ...
    // ============================================================================
    TCanvas* c_heat = new TCanvas("c_momentum_flatness_sim_heatmap",
        "Momentum flatness landscape (sim) — 4 windows", 1900, 500);
    ...
    c_heat->SaveAs("plots/output/scan_momentum_flatness_sim_heatmap.pdf");
    c_heat->SaveAs("plots/output/scan_momentum_flatness_sim_heatmap.png");
    std::cout << "Saved: plots/output/scan_momentum_flatness_sim_heatmap.{pdf,png}\n";
    ...
    for (size_t w = 0; w < kWindows.size(); ++w) {
        ...
        TCanvas* c = new TCanvas(
            Form("c_momentum_flatness_sim_best_%s", kWindows[w].short_id.c_str()),
            Form("Best %s — ep_p_rec>%d  em_p_rec>%d MeV/c",
        ...
        h_PT3->SetTitle(Form(
            "m_{ee}^{sim} at best cuts ep_p_rec>%d em_p_rec>%d MeV/c;M_{e^{+}e^{-}}^{sim} [GeV/c^{2}];weighted entries / 20 MeV",
            b.ep_cut, b.em_cut));
        ...
        tx.DrawLatex(0.16, 0.90,
            Form("best ep_p_rec>%d  em_p_rec>%d MeV/c", b.ep_cut, b.em_cut));
        ...
        const std::string base = Form(
            "plots/output/scan_momentum_flatness_sim_best_%s",
