// _explore_low_fade_005.C
// =========================================================================
// Exploratory macro: scans candidate variable-width binning configurations
// for the LOW-fade region [0.05, 0.14] (90 MeV wide), computing per-bin
// 63 * sig_PT2 / sig_PT3 ratios with CB-aware error propagation.
//
// Each candidate combines DENSE 2.5 MeV bins in [0, 0.05] (20 bins) with a
// variable-width tail in [0.05, 0.14]. We then inspect ONLY the variable
// portion and check smoothness against:
//   - the mean of the last 2 dense bins (centers ~0.045, 0.0475), and
//   - the MID pol0 reference ratio = 1.821.
//
// Usage: root -l -b -q plots/_explore_low_fade_005.C
//
...
// Build edge array: 2.5 MeV bins in [0, 0.05] (20 bins, edges 0..0.050)
// followed by the variable portion (skipping the duplicate 0.050).
std::vector<double> buildEdges(const std::vector<double>& var_edges) {
    std::vector<double> edges;
    edges.reserve(21 + var_edges.size());
    for (int i = 0; i <= 20; ++i) {
        edges.push_back(0.0025 * i);  // 0.000, 0.0025, ... 0.050
    }
    // var_edges starts at 0.05 — skip it to avoid duplicate.
...
ConfigStats runConfig(TTree* t_em, TTree* t_pp, TTree* t_mm,
    ...
    printf("\n==================== %s ====================\n", label.c_str());
    printf("Variable edges in [0.05, 0.14]:");
    ...
    // Reference from the last 2 dense bins (b=19 -> [0.045,0.0475],
    // b=20 -> [0.0475,0.05]).  Inverse-sigma^2 weighted mean.
    ...
    const int b0 = 21;          // first variable bin index (after 20 dense)
    double sum_sig = 0.0;
...
void _explore_low_fade_005() {
    ...
    std::cout << "Dense bins in [0, 0.05]: 20 x 2.5 MeV\n";
    ...
    std::vector<Cfg> cfgs = {
        {"U1", "U1: 1 bin x 90 MeV          [0.05, 0.14]",
            {0.05, 0.14}},
        {"U2", "U2: 2 bins x 45 MeV         [0.05, 0.095, 0.14]",
            {0.05, 0.095, 0.14}},
        {"U3", "U3: 3 bins x 30 MeV         [0.05, 0.08, 0.11, 0.14]",
            {0.05, 0.08, 0.11, 0.14}},
        {"U4", "U4: 4 bins x 22.5 MeV       [0.05, 0.0725, 0.095, 0.1175, 0.14]",
            {0.05, 0.0725, 0.095, 0.1175, 0.14}},
        {"U5", "U5: 5 bins x 18 MeV         [0.05, 0.068, 0.086, 0.104, 0.122, 0.14]",
            {0.05, 0.068, 0.086, 0.104, 0.122, 0.14}},
        {"U6", "U6: 6 bins x 15 MeV         [0.05, 0.065, 0.08, 0.095, 0.11, 0.125, 0.14]",
            {0.05, 0.065, 0.08, 0.095, 0.11, 0.125, 0.14}},
        {"U7", "U7: 7 mixed bins            [0.05, 0.06, 0.07, 0.08, 0.09, 0.10, 0.12, 0.14]",
            {0.05, 0.06, 0.07, 0.08, 0.09, 0.10, 0.12, 0.14}},
        {"U8", "U8: 4 bins (20,20,20,30)    [0.05, 0.07, 0.09, 0.11, 0.14]",
            {0.05, 0.07, 0.09, 0.11, 0.14}},
    };
    ...
    // -----------------------------------------------------------------
    printf("\n==================== SUMMARY ====================\n");
    printf("%-4s  %-55s %5s %5s %5s %5s %8s %8s %8s\n",
           "tag", "label", "nvar", "nbad", "noor", "njmp",
           "dDense", "dMid", "<sig>");
    for (size_t i = 0; i < cfgs.size(); ++i) {
        printf("%-4s  %-55s %5zu %5d %5d %5d %8.3f %8.3f %8.3f\n",
               cfgs[i].tag.c_str(),
               cfgs[i].label.c_str(),
    ...
    // score = dDense + dMid + 0.5*<sigma> + 0.05*nvar + penalties
    // -----------------------------------------------------------------
    int best = -1;
