 ...
 *   output_epem_exp.root            (exp signal, like-sign-pair "all")
 *   output_epep_exp.root            (exp CB++)
 *   output_emem_exp.root            (exp CB--)
 *   output_epem_sim.root            (sim, weighted by sim_genweight)
 *
 ...
 *
 *   auto [a, c, s] = exp.drawSignal("dilepton_nt", "m_ee", 200, 0, 1, "");
 *   auto* h_sim    = sim.draw("dilepton_nt", "m_ee", 200, 0, 1, "");
 *
                          ...
                          double display_lo = 0.0, double display_hi = 0.0)
{
    ...
    leg->AddEntry(h_all, "exp: all (e^{+}e^{-})", "lpe");
...
// ============================================================================
// drawJointSingle — exp single curve (no CB) + sim band + sim step line.
//
// For purely-ECAL observables that don't admit like-sign CB extraction
// (M(γγ) at mult==2, M(γγγγ) at mult==4 in the e+e- analysis). The data
// histogram is drawn as black markers + error bars, in the same style
// produced by PlotUtils::drawNtupleSingle / styleAll.
//
// Drawing order (back → front):
//   1) h_data — black markers + error bars (sets axes)
//   2) h_sim_band — light-gray solid stat-error band (clone of h_sim, "E2")
//   3) h_sim — brown step histogram line ("HIST")
//   4) h_data — re-drawn LAST on top of sim band/step
// ============================================================================
inline TCanvas* drawJointSingle(TH1* h_data, TH1* h_sim,
                                const std::string& canvasTitle,
                                const std::string& canvasName,
                                bool logy = false,
                                double sim_scale = 1.0,
                                double display_lo = 0.0, double display_hi = 0.0,
                                const std::string& dataLegendLabel = "exp: data (e^{+}e^{-})")
{
    TCanvas* c = new TCanvas(canvasName.c_str(), canvasTitle.c_str(), 800, 600);
    c->SetMargin(0.12, 0.05, 0.12, 0.08);
    if (logy) c->SetLogy();
    ...
    if (!canvasTitle.empty()) {
        h_data->SetTitle(canvasTitle.c_str());
    }

    const bool zoom = (display_lo < display_hi);
    int b_lo = 1, b_hi = h_data->GetNbinsX();
    if (zoom) {
        h_data->GetXaxis()->SetRangeUser(display_lo, display_hi);
        b_lo = h_data->FindBin(display_lo);
        b_hi = h_data->FindBin(display_hi);
    }

    // Y-axis cap: scan visible window; include sim+err so the band isn't clipped.
    double y_max = 0.0;
    for (int i = b_lo; i <= b_hi; ++i) {
        y_max = std::max(y_max, h_data->GetBinContent(i));
        if (h_sim) y_max = std::max(y_max, h_sim->GetBinContent(i) + h_sim->GetBinError(i));
    }
    h_data->SetMaximum(y_max * (logy ? 3.0 : 1.2));
    h_data->SetMinimum(logy ? 0.5 : 0.0);
    ...
    // Re-draw data on top so markers aren't covered by sim band/step.
    h_data->Draw("E SAME");

    TLegend* leg = new TLegend(0.55, 0.78, 0.94, 0.90);
    leg->SetBorderSize(0);
    leg->SetFillStyle(0);
    leg->SetTextSize(0.032);
    leg->AddEntry(h_data, dataLegendLabel.c_str(), "lpe");
    if (h_sim) {
        char sim_lbl[128];
        snprintf(sim_lbl, sizeof(sim_lbl), "sim #times %.3g", sim_scale);
        leg->AddEntry(h_sim,      sim_lbl,           "l");
