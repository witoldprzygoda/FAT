// research/plots/compare_yields_pi0.C — exp vs sim shape comparison.
//
// Reads:
//   research/fit_results_<exp_fl>.root   (this branch — exp, default 'rec')
//   research/fit_results.root            (sim leftover from pp45_epem_sim)
//
// Both files come from plots/fit_pi0.C — same TTree schema with
// yield_data_{1,2,3}sig{,_err} per OA slice.
//
// Plot 1 — yield_pi0_compare_norm_1sig.{pdf,png}
//   Normalized yield (μ ± 1σ) vs opening angle, exp & sim overlaid.
//   Each spectrum scaled so Σ_i y_i = 1 — pure shape comparison.
//
// Plot 2 — yield_pi0_compare_ratio_norm.{pdf,png}
//   Three series: ratio (norm_exp / norm_sim) for ±1σ, ±2σ, ±3σ windows.
//   Note: S_exp / S_sim cancels in per-bin ratio formula, so the
//   ratio's relative error reduces to √((σ_y_e/y_e)² + (σ_y_s/y_s)²).
//
// Usage (from research/):
//   root -l -b -q plots/compare_yields_pi0.C            # exp REC vs sim
//   root -l -b -q 'plots/compare_yields_pi0.C("cor")'   # exp COR vs sim
...
namespace {
    struct SliceRow {
        double oa_c, oa_w;          // slice center, half-width
...
void compare_yields_pi0(const char* exp_flavour = "rec") {
    ...
    const std::string path_sim = "fit_results.root";
    ...
    // Sanity check: OA bin centers match.
    for (int i = 0; i < N; ++i) {
    ...
    mg1->SetTitle(TString::Format(
        "Normalized #pi^{0} signal yield (#pm 1#sigma): exp(%s) vs sim;"
    ...
    auto* c1 = new TCanvas("c_norm_1sig", "norm yield 1sig", 1000, 700);
    ...
    // ratio_i = (y_e/S_e) / (y_s/S_s)
    // σ_r / r = √((σ_y_e/y_e)² + (σ_y_s/y_s)²)   — overall (S_e/S_s) is fixed.
    ...
    mg2->SetTitle(TString::Format(
        "Normalized #pi^{0} yield ratio: exp(%s) / sim;"
        "OA(e^{+}e^{-}) [deg];"
        "(y_{exp}/#Sigma_{exp}) / (y_{sim}/#Sigma_{sim})",
        fl.c_str()));

    auto* c2 = new TCanvas("c_ratio", "exp/sim ratio", 1000, 700);
    c2->SetMargin(0.13, 0.05, 0.12, 0.08);
    c2->SetGrid();
    mg2->Draw("A");
    mg2->GetXaxis()->SetLimits(0.0, 10.0);

    // Reference line at ratio = 1 (perfect shape match).
    ...
    std::cout << "Wrote:\n"
              << "  " << out1 << ".{pdf,png}\n"
              << "  " << out2 << ".{pdf,png}\n"
              << "  (" << N << " OA slices, exp totals: "
              << "S_1σ=" << S_e1 << " S_2σ=" << S_e2 << " S_3σ=" << S_e3 << ")\n"
              << "  (sim totals:                         "
              << "S_1σ=" << S_s1 << " S_2σ=" << S_s2 << " S_3σ=" << S_s3 << ")\n";
}
