// mass_ee_padnum_flatness_scan_sim.C — 2D scan of (ep_rich_padnum,
// em_rich_padnum) cuts on the SIM sample to find combinations that
// FLATTEN the PT2/PT3 trigger-ratio modulation in m_ee_sim space.
//
...
//   Default grid: cut ∈ [5, 20] (16 values per leg → 256 combinations).
//
// Mass windows:
...
//   plots/output/scan_padnum_flatness_sim_heatmap.{pdf,png}
//   plots/output/scan_padnum_flatness_sim_best_<window>.{pdf,png}  (×4)
//
...
//
// Usage:
//   root -l -b -q plots/mass_ee_padnum_flatness_scan_sim.C
//   root -l -b -q 'plots/mass_ee_padnum_flatness_scan_sim.C(5, 20)'
//
...
void mass_ee_padnum_flatness_scan_sim(int cut_min = 5, int cut_max = 20) {
    gStyle->SetOptStat(0);
    gStyle->SetOptFit(0);
    ...
    for (size_t w = 0; w < kWindows.size(); ++w) {
        h_chi2[w][0] = new TH2D(
            ...
            Form("Flatness #chi^{2}/ndf — %s;ep_rich_padnum #geq;em_rich_padnum #geq;#chi^{2}/ndf",
    ...
    // ============================================================================
    TCanvas* c_heat = new TCanvas("c_padnum_flatness_sim_heatmap",
        "Padnum flatness landscape (sim) — 4 windows", 1900, 500);
    ...
    c_heat->SaveAs("plots/output/scan_padnum_flatness_sim_heatmap.pdf");
    c_heat->SaveAs("plots/output/scan_padnum_flatness_sim_heatmap.png");
    std::cout << "Saved: plots/output/scan_padnum_flatness_sim_heatmap.{pdf,png}\n";
    ...
    for (size_t w = 0; w < kWindows.size(); ++w) {
        ...
        TCanvas* c = new TCanvas(
            Form("c_padnum_flatness_best_%s", kWindows[w].short_id.c_str()),
            Form("Best %s — ep_padnum>=%d  em_padnum>=%d",
        ...
        h_PT3->SetTitle(Form(
            "m_{ee}^{sim} at best cuts ep#geq%d em#geq%d;M_{e^{+}e^{-}}^{sim} [GeV/c^{2}];weighted entries / 20 MeV",
            b.ep_cut, b.em_cut));
        ...
        tx.DrawLatex(0.16, 0.90,
            Form("best ep#geq%d  em#geq%d", b.ep_cut, b.em_cut));
        ...
        const std::string base = Form(
            "plots/output/scan_padnum_flatness_sim_best_%s",
