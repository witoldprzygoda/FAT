...
// =========================================================================
// Loop a single channel's meson_dalitz_nt and fill per-cell mass histos
// using the ECAL-corrected mass.
// =========================================================================
...
void ecal_pi0_2dscan_corr(const char* flavour = "rec") {
    ...
    auto cells_pp  = makeCells("pp");
    auto cells_mm  = makeCells("mm");

    std::cout << "ECAL π⁰ corrected 2D scan: flavour=" << fl
              << "  cells=" << (nE * nTh) << "\n";
    std::cout << "Map: " << map_path << " (" << map_hist << ")\n";

    fillChannel("../output_epem_exp.root", mass_var, look, cells_all, "all");
    fillChannel("../output_epep_exp.root", mass_var, look, cells_pp,  "pp ");
    fillChannel("../output_emem_exp.root", mass_var, look, cells_mm,  "mm ");

    // Build CB-subtracted signal per cell, fit, fill heatmap.
    auto buildSignalCorr = [&](TH1D* h_all, TH1D* h_pp, TH1D* h_mm,
                               const std::string& tag) -> TH1D* {
        TH1D* h_cb = (TH1D*)h_all->Clone(("h_cb_corr_" + tag).c_str());
        h_cb->Reset();
    ...
    for (int ei = 0; ei < nE; ++ei) {
        for (int ti = 0; ti < nTh; ++ti) {
            ...
            TH1D* h_sig = buildSignalCorr(cells_all[ei][ti],
                                           cells_pp[ei][ti],
                                           cells_mm[ei][ti], tag);
    ...
    std::cout << "\nWrote:\n"
              ...
              << "  plots/output/ecal_pi0_2dscan_corr_{mu,dmu}_" << fl << ".{pdf,png}\n";
}
