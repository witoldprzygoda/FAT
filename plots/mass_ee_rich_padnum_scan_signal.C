// mass_ee_rich_padnum_scan_signal.C — sister of
// mass_ee_rich_padnum_scan_significance.C, but the optimization OBJECTIVE
// is the CB-subtracted SIGNAL YIELD (not significance).
//
// Per-window objective maximized at each scan point: S_PT2 + S_PT3 inside
// the m_ee window, where S = N_epem − CB and CB = 2·√(N_++·N_−−).
//
// TOP criterion uses the same range as the significance variant
// (above-π⁰, [0.14, 1.4] GeV) but the figure of merit is now top_S_sum
// = S_PT2[0.14-1.4] + S_PT3[0.14-1.4]. The window with the highest
// top_S_sum gets the ★ TOP mark.
//
// All output files have `_signal` infix so they don't overwrite the
// significance-criterion outputs.
//
// ---------------------------------------------------------------------
// Original significance-variant header preserved below for reference:
// ---------------------------------------------------------------------
// mass_ee_rich_padnum_scan_significance.C — 2D scan of (ep_rich_padnum,
// em_rich_padnum) cuts to maximize CB-subtracted signal significance.
//
// What is scanned:
//   • EpEm pair (signal candidates): asymmetric cut grid
//       ep_rich_padnum >= ep_cut    AND
//       em_rich_padnum >= em_cut
//   • EpEp pair (like-sign + + for CB): SYNCHRONIZED cut on ep_cut
//       (both legs must pass ep_cut — both legs are e+)
//   • EmEm pair (like-sign − − for CB): SYNCHRONIZED cut on em_cut
//       (both legs must pass em_cut — both legs are e−)
//   This synchronization is mandatory: the e+ leg cut in EpEm must equal
//   the cut applied in EpEp, and the e− leg cut in EpEm must equal the
//   cut applied in EmEm — otherwise the CB combinatorial would not match
//   the EpEm sample population.
//
// Default cut grid: cut ∈ [5, 20] (16 values per leg → 256 combinations).
//
...
//   1.   plots/output/scan_rich_padnum_heatmap_significance.{pdf,png}
...
//
//   2-5. plots/output/scan_rich_padnum_best_<window>.{pdf,png}
...
//
// Usage:
//   root -l -b -q plots/mass_ee_rich_padnum_scan_significance.C
//   root -l -b -q 'plots/mass_ee_rich_padnum_scan_significance.C(5, 20)'
//
...
struct BestPoint {
    ...
    double S_sum          = 0.0;   // S_PT2 + S_PT3  — OBJECTIVE being maximized
    double top_S_sum      = 0.0;   // top_S_PT2 + top_S_PT3 — used to pick TOP
    bool   is_top         = false; // window with max top_S_sum
};
...
void mass_ee_rich_padnum_scan_signal(int cut_min = 5,
                                     int cut_max = 20) {
    gStyle->SetOptStat(0);
    gStyle->SetOptFit(0);
    ...
    std::cout << "Scanning ep_rich_padnum, em_rich_padnum on [" << cut_min
              << ", " << cut_max << "]  (n_cuts=" << n_cuts << ", "
    ...
    Long64_t n_em_total = 0, n_em_use = 0;
    {
        ...
        while (r.Next()) {
            ...
            if (trg < 0) continue;
            const int P_ep = static_cast<int>(*ep_pad);
            const int P_em = static_cast<int>(*em_pad);
    ...
    Long64_t n_pp_total = 0, n_pp_use = 0;
    {
        ...
        while (r.Next()) {
            ...
            const int trg = trgFromBit(static_cast<int>(*trigb));
            if (trg < 0) continue;
            const int P1 = static_cast<int>(*ep_pad);
            const int P2 = static_cast<int>(*em_pad);
            const int max_c = std::min(cut_max, std::min(P1, P2));
            if (max_c < cut_min) continue;
            ++n_pp_use;
            const double m = *mee;
            for (int e = cut_min; e <= max_c; ++e) {
    ...
    for (size_t w = 0; w < kWindows.size(); ++w) {
        for (int t = 0; t < NUM_TRG; ++t) {
            auto* h = new TH2D(
                ...
                Form("Significance — %s / %s;ep_rich_padnum #geq;em_rich_padnum #geq;S / #sqrt{S + 2B}",
    ...
    for (int ei = 0; ei < n_cuts; ++ei) {
        for (int mi = 0; mi < n_cuts; ++mi) {
            ...
            for (size_t w = 0; w < kWindows.size(); ++w) {
                ...
                const double sum_S = Sw[TRG_PT2] + Sw[TRG_PT3];

                if (sum_S > best[w].S_sum) {
    ...
    // ============================================================================
    {
        for (size_t w = 0; w < kWindows.size(); ++w) {
            ...
            b.top_S_sum = b.top_S[TRG_PT2] + b.top_S[TRG_PT3];
        }
        double max_top = -1.0;
        size_t tw = 0;
        for (size_t w = 0; w < kWindows.size(); ++w)
            if (best[w].top_S_sum > max_top) {
                max_top = best[w].top_S_sum;
                tw = w;
            }
        best[tw].is_top = true;
        std::cout << "TOP criterion (range [" << kTopLo << ", " << kTopHi
                  << "] GeV, signal sum PT2+PT3): window "
    ...
    // ============================================================================
    std::cout << "\n=== Best (ep_cut, em_cut) per window — OBJECTIVE: max S_sum ===\n";
    std::cout << "  (★ = max top_S_sum in [" << kTopLo << ", " << kTopHi
              << "] GeV across all 4 windows)\n\n";
    ...
    for (size_t w = 0; w < kWindows.size(); ++w) {
        ...
        // Per-window summary line (objective = sum of S; reported in S col,
        // significance shown as derived from sum).
        std::cout
            << "                          sum |  "
            << pad(fmtApos(b.S_sum),   12) << "  "
            << pad("",                12) << "  "
            << pad("",                  9) << "  |  "
            << pad(fmtApos(b.top_S_sum),12) << "  "
            << pad("",                12) << "  "
            << pad("",                  9) << "\n";
    }
    std::cout << "\n";
    ...
    TCanvas* c_heat = new TCanvas("c_scan_heatmap_signal",
        "Significance landscape (S-objective scan) — 4 windows × 2 triggers",
        1900, 900);
        ...
        for (size_t w = 0; w < kWindows.size(); ++w) {
            ...
            if (b.is_top && t == TRG_PT3) {
                ...
                tx.DrawLatex(0.50, 0.92,
                    Form("#bigstar TOP (S_sum=%.0f)", b.top_S_sum));
            }
        }
    }
    c_heat->SaveAs("plots/output/scan_rich_padnum_heatmap_signal.pdf");
    c_heat->SaveAs("plots/output/scan_rich_padnum_heatmap_signal.png");
    std::cout << "  saved: plots/output/scan_rich_padnum_heatmap_signal.{pdf,png}\n";
    ...
    for (size_t w = 0; w < kWindows.size(); ++w) {
        ...
        TCanvas* c = new TCanvas(
            Form("c_best_%s_signal", kWindows[w].short_id.c_str()),
            Form("Best %s (S-objective) — ep_cut=%d, em_cut=%d",
        ...
        auto drawMeePanel = [&](int pad_idx, int t) {
            ...
            h_all[t]->SetTitle(Form(
                "m_{ee} at %s (best cuts ep#geq%d em#geq%d);"
            ...
            if (b.is_top && t == TRG_PT3) {
                ...
                tex.DrawLatex(0.18, 0.18,
                    Form("(top_S_sum in [%.2f,%.2f] = %.0f)",
                         kTopLo, kTopHi, b.top_S_sum));
            }
        };
        ...
        const std::string base = Form(
            "plots/output/scan_rich_padnum_best_%s_signal",
