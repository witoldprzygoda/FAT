/**
 * @file plot_ppip_miss_mass.C
 * @brief Plot ppip missing mass comparison
 *
 * Displays ppip_miss_mass from three files
 * File 1 (black): output_fwd_ppip.root
 * File 2 (blue): output_fwd_ppip_full.root
 * File 3 (red): output_fwd_ppip_neutroncut.root
 */

void plot_ppip_miss_mass() {
    gStyle->SetOptStat(0);
    ...
    TH1D* h1 = (TH1D*)f1->Get("ppip/ppip_miss_mass");
    TH1D* h2 = (TH1D*)f2->Get("ppip/ppip_miss_mass");
    TH1D* h3 = (TH1D*)f3->Get("ppip/ppip_miss_mass");

    if (!h1 || !h2 || !h3) {
        std::cerr << "Error: Could not get histogram ppip_miss_mass!" << std::endl;
        return;
    }

    // Create canvas
    TCanvas* c1 = new TCanvas("c1", "MM(p#pi^{+}) Missing Mass Comparison", 800, 600);
    ...
    h3->SetLineWidth(1);
    h3->Scale(100.0);  // Scale for visibility

    // Set titles and labels
    h1->SetTitle("MM(p#pi^{+}) Missing Mass");
    h1->GetXaxis()->SetTitle("MM(p#pi^{+}) [GeV/c^{2}]");
    ...
    c1->Update();
    // c1->SaveAs("plot_ppip_miss_mass.pdf");
    // c1->SaveAs("plot_ppip_miss_mass.png");
