/**
 * @file plot_pwa_groupB.C
 * @brief Plot PWA Group B: Momentum distributions in LAB frame
 *
 * Displays pwa_pip_p, pwa_p_p, pwa_n_p from three files
 * File 1 (black): output_fwd_ppip.root
 * File 2 (blue): output_fwd_ppip_full.root
 * File 3 (red): output_fwd_ppip_neutroncut.root
 */

void plot_pwa_groupB() {
    gStyle->SetOptStat(0);
    ...
    // Histogram names for Group B
    const char* hist_names[3] = {"pwa/pwa_pip_p", "pwa/pwa_p_p", "pwa/pwa_n_p"};
    const char* titles[3] = {"#pi^{+} momentum", "Proton momentum", "Neutron momentum"};

    // Create canvas with 3 horizontal pads
    TCanvas* c1 = new TCanvas("c1", "PWA Group B: Momentum distributions", 1800, 600);
    c1->Divide(3, 1);
    ...
    c1->Update();
    // c1->SaveAs("plot_pwa_groupB.pdf");
    // c1->SaveAs("plot_pwa_groupB.png");
