...
namespace {
    ...
    // (conversions/material) so we shrink the lower edge.
    ...
    constexpr double kAlphaTightMaxMid = 1.65;   // 3.8 ≤ OA < 9.0
    constexpr double kAlphaTightMaxHi  = 2.0;    // OA ≥ 9.0
    constexpr double kOA_EnableP3      = 9.0;    // ≥ this: free a₃·m³ term
...
// ============================================================================
// Full model: CB + (P2 or P3) + exp. par[10] = a3, fixed=0 below kOA_EnableP3.
// ============================================================================
                  ...
                  std::vector<TCanvas*>& canvases_res)
{
    ...
    fit->SetParameter(5, 100.0);
    fit->SetParameter(6, -500.0);
    fit->SetParameter(7, 500.0);
    fit->SetParameter(8, 1000.0);
