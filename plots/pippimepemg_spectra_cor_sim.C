...
//       sample and overlay the M(pippimepem) of the pippimepemg-tagged subset
//       rescaled to match the full-sample integral in the M ~ [0.45, 0.55] GeV
//       window. (Same observable, two event subsets — shapes should match.)
//   (B) STANDALONE — M(pippimepemg) under the same cut configurations.
//
...
void pippimepemg_spectra_cor_sim() {
    ...
    // -----------------------------------------------------------------
    // SECTION A: OVERLAY — M(pippimepem), full sample vs pippimepemg-tagged
    // -----------------------------------------------------------------
    auto drawOverlay = [&](const std::string& cut_base, const std::string& title,
        ...
        auto* h_tagged = pu.drawNtupleSingle(
            NT, "m_pippimepem", nbins, 0.2, 1.4, wcut(cut_tagged),
            ";M_{#pi^{+}#pi^{-}e^{+}e^{-}} [GeV/c^{2}];a.u.");
        ...
        leg->AddEntry(h_full,   "M(#pi^{+}#pi^{-}e^{+}e^{-}) full sample", "lpe");
    ...
    // -----------------------------------------------------------------
    // SECTION B: STANDALONE — M(pippimepemg)
    // -----------------------------------------------------------------
