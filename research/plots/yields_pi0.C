// research/plots/yields_pi0.C — derived plots from fit_results_<fl>.root.
//
// Reads research/fit_results_<flavour>.root produced by plots/fit_pi0.C and
// makes:
//
//   1) yield_pi0_<fl>_vs_oa.{pdf,png}
...
//
// Usage (from research/):
//   root -l -b -q plots/yields_pi0.C            # REC (default)
//   root -l -b -q 'plots/yields_pi0.C("cor")'   # COR
...
void yields_pi0(const char* flavour = "rec",
    ...
    const std::string fpath = "fit_results_" + fl + sfx + ".root";
    ...
    mg->SetTitle(TString::Format(
        "#pi^{0} signal yield vs opening angle (%s);"
    ...
    c1->Update();
    const std::string out1 = "plots/output/yield_pi0_" + fl + sfx + "_vs_oa";
    ...
    gm->SetTitle(TString::Format(
        "#pi^{0} peak position vs opening angle (%s);"
        "OA(e^{+}e^{-}) [deg];"
    ...
    c2->Update();
    const std::string out2 = "plots/output/mu_pi0_" + fl + sfx + "_vs_oa";
