// mass_ee_pt_ratio_purity_cascade_sim_linfit.C
// =========================================================================
// LINEAR FIT (a + b·x) version of the purity cascade study on sim m_ee
// spectrum and PT2/PT3 trigger ratio.
//
// Identical to mass_ee_pt_ratio_purity_cascade_sim.C except:
//   * Fit function is LINEAR  "[0] + [1]*x"   (a = intercept, b = slope)
//   * Fit range is [0.15, 1.0] GeV/c²  (vs. [0.1, 1.1] in the constant-fit
//     version) — slightly tighter to avoid threshold edges and the high-mass
//     tail where statistics are very thin.
//
// Interpretation:
//   * A linear fit ABSORBS any LINEAR trend present in the PT2/PT3 ratio.
//     Therefore the chi²/ndf of this fit measures ONLY the residual,
//     NON-LINEAR modulation of the ratio (sinusoidal, bumps, threshold
//     effects, etc.).
//   * Lower chi²/ndf ⇒ the ratio is well-described by a straight line
//     (any deviation from y=const is mostly a smooth slope).
//   * High chi²/ndf, comparable to the constant-fit value ⇒ the
//     modulation is genuinely non-linear (e.g. sinusoidal structure
//     near vector-meson masses) and cannot be absorbed by a slope.
//
// Comparison with the constant-fit companion macro
//   (mass_ee_pt_ratio_purity_cascade_sim.C):
//     * if linear χ²/ndf ≈ constant χ²/ndf  →  no significant linear trend;
//       the modulation is intrinsically non-linear.
//     * if linear χ²/ndf ≪ constant χ²/ndf  →  there IS a strong linear
//       component; the constant fit was being driven by an overall tilt.
//
...
//
// Output:
//   plots/output/mass_ee_pt_ratio_purity_cascade_sim_linfit_step0_raw_truth.{pdf,png}
//   plots/output/mass_ee_pt_ratio_purity_cascade_sim_linfit_step1_raw.{pdf,png}
//   plots/output/mass_ee_pt_ratio_purity_cascade_sim_linfit_step2_vertex.{pdf,png}
//   plots/output/mass_ee_pt_ratio_purity_cascade_sim_linfit_step3_isBest.{pdf,png}
//   plots/output/mass_ee_pt_ratio_purity_cascade_sim_linfit_step4_simID.{pdf,png}
//   plots/output/mass_ee_pt_ratio_purity_cascade_sim_linfit_step5_samevtx.{pdf,png}
//   plots/output/mass_ee_pt_ratio_purity_cascade_sim_linfit_overlay.{pdf,png}
//
// Usage:
//   root -l -b -q plots/mass_ee_pt_ratio_purity_cascade_sim_linfit.C
//
...
struct FitResult {
    double a;          // y-intercept (param 0)
    double a_err;
    double b;          // slope (param 1)
    double b_err;
    double chi2_ndf;
    int    ndf;
};

FitResult fitLinear(TH1D* h, double xlo, double xhi, Color_t color) {
    const std::string fname = std::string(h->GetName()) + "_fl";
...
void mass_ee_pt_ratio_purity_cascade_sim_linfit() {
    gStyle->SetOptStat(0);
    gStyle->SetOptFit(0);
    ...
    std::cout << "Fit function: linear  a + b*x\n";
    ...
    for (size_t i = 0; i < kScenarios.size(); ++i) {
        ...
        FitResult fr = fitLinear(r, kFitLo, kFitHi, kColFit);
        printf("  linear fit [%.2f, %.2f]:  a = %.4f ± %.4f   b = %.4f ± %.4f   chi2/ndf = %.1f / %d = %.2f\n",
               kFitLo, kFitHi,
        ...
        TCanvas* c = new TCanvas(
            Form("c_purity_cascade_linfit_%s", S.id.c_str()),
            S.label.c_str(),
            1500, 600);
        ...
        tx.DrawLatex(0.16, 0.86,
            Form("linear fit [%.2f, %.2f]:  a = %.4f #pm %.4f   b = %.4f #pm %.4f",
                 kFitLo, kFitHi, fr.a, fr.a_err, fr.b, fr.b_err));
        ...
        const std::string base =
            std::string("plots/output/mass_ee_pt_ratio_purity_cascade_sim_linfit_") + S.id;
    ...
    // ============================================================================
    TCanvas* c_ovl = new TCanvas("c_purity_cascade_linfit_overlay",
        "All 6 ratios overlaid (linear fit)", 1500, 700);
    gPad->SetMargin(0.10, 0.04, 0.13, 0.10);
    ratios_for_overlay[0]->SetTitle(
        "PT2/PT3 cumulative purity cascade — sim (linear fit);"
    ...
    for (size_t i = 0; i < kScenarios.size(); ++i) {
        leg->AddEntry(ratios_for_overlay[i],
            Form("%s   a=%.3f b=%.3f #chi^{2}/ndf=%.1f",
                 kScenarios[i].label.c_str(),
                 fits[i].a, fits[i].b, fits[i].chi2_ndf),
            "lpe");
    }
    leg->Draw();

    c_ovl->SaveAs("plots/output/mass_ee_pt_ratio_purity_cascade_sim_linfit_overlay.pdf");
    c_ovl->SaveAs("plots/output/mass_ee_pt_ratio_purity_cascade_sim_linfit_overlay.png");

    // ============================================================================
    // Summary table
    // ============================================================================
    std::cout << "\n\n=== SUMMARY (linear fit  a + b*x  in [" << kFitLo << ", " << kFitHi << "]) ===\n";
    printf("  %-50s  %-12s  %-12s  %-18s  %-18s  %-15s\n",
           "scenario", "PT3 yield", "PT2 yield",
           "a (intercept)", "b (slope)", "chi2/ndf");
    std::cout << "  " << std::string(135, '-') << "\n";
    for (size_t i = 0; i < kScenarios.size(); ++i) {
        printf("  %-50s  %12.0f  %12.0f  %.4f±%.4f  %+.4f±%.4f  %.1f / %d = %.2f\n",
               kScenarios[i].label.c_str(),
               yields_PT3[i], yields_PT2[i],
               fits[i].a, fits[i].a_err,
               fits[i].b, fits[i].b_err,
