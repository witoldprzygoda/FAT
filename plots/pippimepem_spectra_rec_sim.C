// pippimepem_spectra_rec_sim.C — pi+pi-e+e- observables for SIMULATION (no CB).
...
//
// Usage: root -l -b -q plots/pippimepem_spectra_rec_sim.C
...
void printIntegral_rec_sim(const char* label, TH1D* h) {
    std::cout << "\n=== " << label << " ===\n";
    std::cout << "  entries = " << h->GetEntries()
              << "   integral = " << h->Integral() << "\n";
}

void pippimepem_spectra_rec_sim() {
    ...
    auto* h1 = pu.drawNtupleSingle(
        ...
        160, 0.0, 0.8, wcut(""),
        ";M_{e^{+}e^{-}} [GeV/c^{2}];a.u.");
    auto* cv1 = pu.drawSingle(h1, "M_{e^{+}e^{-}}", "c_m_ee_rec_sim", /*logy=*/true);
    pu.save(cv1, "m_ee_rec_sim");
    printIntegral_rec_sim("M(e+e-)", h1);
    ...
    auto* cv2 = pu.drawSingle(h2, "M_{#pi^{+}#pi^{-}}", "c_m_pippim_rec_sim");
    pu.save(cv2, "m_pippim_rec_sim");
    printIntegral_rec_sim("M(pi+pi-)", h2);
    ...
    auto* cv3 = pu.drawSingle(h3,
                              "M_{#pi^{+}#pi^{-}e^{+}e^{-}}", "c_m_pippimepem_rec_sim");
    pu.save(cv3, "m_pippimepem_rec_sim");
    printIntegral_rec_sim("M(pi+pi-e+e-)", h3);
    ...
    auto* cv4 = pu.drawSingle(h4, "MM(#pi^{+}#pi^{-})", "c_mm_pippim_rec_sim");
    pu.save(cv4, "mm_pippim_rec_sim");
    printIntegral_rec_sim("MM(pi+pi-)", h4);
    ...
    auto* cv5 = pu.drawSingle(h5, "MM(#pi^{+}#pi^{-}e^{+}e^{-})",
                              "c_mm_pippimepem_rec_sim");
    pu.save(cv5, "mm_pippimepem_rec_sim");
    printIntegral_rec_sim("MM(pi+pi-e+e-)", h5);
    ...
    // --- 5a-5c. MM observables after cut_2d -------------------------------
    {
        auto* h = pu.drawNtupleSingle(
            ...
            ";MM(e^{+}e^{-}) [GeV/c^{2}];a.u.");
        auto* cv = pu.drawSingle(h, "MM(e^{+}e^{-}) after cut_2d", "c_mm_epem_cut2d_rec_sim");
        pu.save(cv, "mm_epem_cut2d_rec_sim");
        printIntegral_rec_sim("MM(e+e-) after cut_2d", h);
    }
    {
        auto* h = pu.drawNtupleSingle(
            "pippimepem_nt", "mm_pippim",
            200, 0.0, 4.0, wcut("cut2d_pass==1"),
            ";MM(#pi^{+}#pi^{-}) [GeV/c^{2}];a.u.");
        auto* cv = pu.drawSingle(h, "MM(#pi^{+}#pi^{-}) after cut_2d",
                                 "c_mm_pippim_cut2d_rec_sim");
        pu.save(cv, "mm_pippim_cut2d_rec_sim");
        printIntegral_rec_sim("MM(pi+pi-) after cut_2d", h);
    }
    {
        auto* h = pu.drawNtupleSingle(
            "pippimepem_nt", "mm_pippimepem",
        ...
        auto* cv = pu.drawSingle(h, "MM(#pi^{+}#pi^{-}e^{+}e^{-}) after cut_2d",
                                 "c_mm_pippimepem_cut2d_rec_sim");
        pu.save(cv, "mm_pippimepem_cut2d_rec_sim");
        printIntegral_rec_sim("MM(pi+pi-e+e-) after cut_2d", h);
    }
    ...
    // (LAB < 50 deg, eta-rest > 140 deg) can be read off directly.
    {
        ...
        auto* cv = pu.drawSingle(h,
                                 "OA_{LAB}((#pi^{+}#pi^{-}),(e^{+}e^{-}))",
                                 "c_oa_pippim_epem_lab_rec_sim");
        pu.save(cv, "oa_pippim_epem_lab_rec_sim");
        printIntegral_rec_sim("OA_LAB((pi+pi-),(e+e-))", h);
    }
    {
        auto* h = pu.drawNtupleSingle(
        ...
        auto* cv = pu.drawSingle(h,
                                 ...
                                 "c_oa_pippim_epem_eta_rest_rec_sim");
        pu.save(cv, "oa_pippim_epem_eta_rest_rec_sim");
        printIntegral_rec_sim("OA_eta_rest((pi+pi-),(e+e-))", h);
    }
    ...
    auto* cv6 = pu.drawSingle(h6,
                              ...
                              "c_m_pippimepem_selected_rec_sim");
    pu.save(cv6, "m_pippimepem_selected_rec_sim");
    printIntegral_rec_sim("M(pi+pi-e+e-) after selection", h6);
    ...
    // --- 6a. Same as plot 6 but with cut_2d additionally applied ----------
    {
        ...
        auto* cv = pu.drawSingle(h,
                                 ...
                                 "c_m_pippimepem_selected_cut2d_rec_sim");
        pu.save(cv, "m_pippimepem_selected_cut2d_rec_sim");
        printIntegral_rec_sim("M(pi+pi-e+e-) after selection + cut_2d", h);
    }
    ...
    auto drawSlice = [&](double lo, double hi, const std::string& tag, bool cut2d = false) {
        ...
        std::string suffix = tag + (cut2d ? "_cut2d" : "") + "_rec_sim";
        ...
        lbl << "M(pi+pi-e+e-) sel" << (cut2d ? "+cut_2d" : "")
            << ", MM in [" << lo << ", " << hi << "]";
        printIntegral_rec_sim(lbl.str().c_str(), h);
    };
    ...
    std::cout << "\nDone. Check plots/output/ (suffix _rec_sim)\n";
}
